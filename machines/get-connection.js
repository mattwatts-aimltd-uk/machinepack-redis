module.exports = {
  ...
  inputs: {
    ...
    manager: {
      ...
      description:
        "The connection manager instance to acquire the connection from.",
      extendedDescription:
    ...
    timeout: {
      ...
      description:
        "The amount of time, in milliseconds, to allow for a successful connection to take place.",
      example: 10000,
      defaultsTo: 60000,
    },
    //
  ...
  exits: {
    ...
    success: {
      ...
      extendedDescription:
        "This connection should be eventually released.  Otherwise, it may time out.  " +
    ...
    failed: {
      description:
        "Could not acquire a connection to the database using the specified manager.",
      extendedDescription:
  ...
  fn: function (inputs, exits) {
    ...
    try {
      client = redis.createClient(
        inputs.manager.connectionString,
        redisClientOptions
      );
    } catch (e) {
      ...
      if (e.name === "TypeError") {
        return exits.failed({
          error: new Error(
            "Invalid Redis client options in manager. Details: " + e.stack
          ),
        });
      }
    ...
    function onPreConnectionError(err) {
      ...
      if (err.command === "AUTH" && err.code === "WRONGPASS") {
        ...
        return exits.failed({
          error: flaverr(
            "ERR_BAD_PASSWORD",
            new Error(
              "The password supplied to the Redis server was incorrect."
            )
          ),
        });
      }
      ...
      if (
        err.command === "INFO" &&
        err.code === "NOAUTH" &&
        (!inputs.meta || inputs.meta.authLater === true)
      ) {
        client.removeListener("end", onPreConnectionEnd);
        client.removeListener("error", onPreConnectionError);
        // Swallow follow-on errors.
        ...
        return exits.failed({
          error: flaverr(
            "ERR_NO_PASSWORD",
            new Error(
              "The Redis server requires a password, but none was supplied."
            )
          ),
        });
      }
    ...
    function onPreConnectionEnd() {
      ...
      return exits.failed({
        error:
          ...
          new Error(
            'Redis client fired "end" event before it finished connecting.'
          ),
      });
    }
    ...
    redisConnectionTimeout = setTimeout(() => {
      return exits.error(
        flaverr(
          "E_REDIS_CONNECTION_TIMED_OUT",
          new Error(
            "Took too long to connect to the specified Redis session server.\nYou can change the allowed connection time by setting the `timeout` input (currently " +
              inputs.timeout +
              "ms)."
          )
        )
    ...
    client.once("ready", () => {
        ...
        if (err) {
          ...
          if (/ECONNREFUSED/g.test(err)) {
            errToSend.message =
              ...
              "is happening in the background). Currently there are " +
              client.connections +
              " underlying Redis connections.\n" +
              "Error details:" +
              err.stack;
          } else {
            errToSend.message =
              "Error emitted from Redis client.\nError details:" + err.stack;
          }
        } else {
          errToSend.message =
      ...
      if (client._fromWLManager) {
        return exits.error(
          new Error(
            "Consistency violation: Somehow, a `_fromWLManager` key already exists on this Redis client instance!"
          )
        );
      }
